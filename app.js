...
function loadNextFiles() {
    ...
    for (let i = 0; i < 2; i++) {
        if (fileIndex < fileArray.length) {
            ...
            const fileContent = iframeDocument.createElement('div');
            fileContent.innerHTML = `
                <h2>${fileName}</h2>
                <iframe src="content/Coding_Study_Wiki_acb5908e8a664382af6667917c0e6cf2/${fileName}" width="100%" height="300px"></iframe>
            `;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    iframe.onload = () => {
        ...
        // Create a sentinel element at the bottom of the iframe to observe
        const sentinel = iframeDocument.createElement('div');
        sentinel.id = 'sentinel';
        sentinel.style.height = '1px';  // Make it small and invisible
        iframeDocument.body.appendChild(sentinel);

        // Use the iframe's document as the root for the IntersectionObserver
        let options = {
            root: iframeDocument,
            rootMargin: "0px",
            threshold: 0.1
        ...
        iframe.contentWindow.addEventListener('scroll', () => {
            ...
            // If scrolled to bottom, load more content
    ...
    // Set initial content of the iframe
